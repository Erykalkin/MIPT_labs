...
Заметим, что обратноупорядоченный массив для этого алгоритма является одним из лучших случаев.
...
![](https://github.com/Erykalkin/MIPT_labs/blob/main/lab_2/L.png)

Видим, что Quicksort с медианой повторяет за BubbleSort. Однако мы уже видели, что его ассимптотика не квадрат.

---
## Вывод
1. Лабы - зло.
2. Написать свою сортировку лечге, чем понять, как работает std::qsort
3. Недетерминированный QuickSort работает быстрее всех среди исследованных алгоритмов. Однако, умные люди говорят, что всегда можно подобрать тест, который убьёт эту сортировку. Мы не увидели вырождения QuickSort в квадрат, потому что наихудший вариант для этого алгоритма - не обратноупорядоченный массив. Для детерминированного алгоритма такой ловушки нет (вроде бы).
4. Я узнал много нового.
